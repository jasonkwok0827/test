    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(max_timestep,timestep,
                                epsilon_min,epsilon_start,exploration_fraction):
            ### PUT YOUR CODE HERE ###
            slope = - ((epsilon_start - epsilon_min)/exploration_fraction) /  max_timestep
            intercept = epsilon_start
            eps = max(epsilon_min, intercept + slope * timestep)
            return eps
            raise(NotImplementedError)


        def epsilon_exponential_decay(max_timestep,timestep,
                        epsilon_min,epsilon_start,epsilon_exponential_decay_factor):
            ### PUT YOUR CODE HERE ###
            factor = (epsilon_exponential_decay_factor ** (1 / max_timestep))**timestep
            return max(epsilon_min, epsilon_start * factor)
        ...
        if self.epsilon_decay_strategy == "constant":
            ...
            # linear decay
            ### PUT YOUR CODE HERE ###
            self.epsilon = epsilon_linear_decay(max_timestep,timestep,
                                                self.epsilon_min,self.epsilon_start,self.exploration_fraction)
        elif self.epsilon_decay_strategy == "exponential":
            # exponential decay
            ### PUT YOUR CODE HERE ###
            self.epsilon = epsilon_exponential_decay(max_timestep,timestep,
                        self.epsilon_min,self.epsilon_start,self.epsilon_exponential_decay_factor)
        else:
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        """
        ### PUT YOUR CODE HERE ###
        action_space,critics_net = self.action_space,self.critics_net
        observed = critics_net(torch.tensor(obs).float())

        #epsilon greedy if explore is true
        if explore:
            if np.random.random() < self.epsilon:
                return action_space.sample()
            else:
                #greedy action
                return np.argmax(observed.detach().numpy())
        else:
            #greedy action
            return np.argmax(observed.detach().numpy())
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        #extract the batch
        states,actions,rewards,next_states,done = getattr(batch, 'states'),getattr(batch, 'actions'),\
            getattr(batch, 'rewards'),getattr(batch, 'next_states'),getattr(batch, 'done')
        #calculate the loss
        q_max = self.critics_target(next_states).max(1)[0].detach().unsqueeze(1)
        loss_func = torch.nn.functional.mse_loss(rewards + (1 - done) * self.gamma * q_max,
                                                 self.critics_net(states).gather(dim=1, index=actions.type(torch.int64)))
        #update the network
        loss_func.backward()
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        """
        ### PUT YOUR CODE HERE ###
        pdf = self.policy.forward(torch.from_numpy(obs).float()).detach().numpy()
        if not explore:
            return np.argmax(pdf)
        else:
            return np.random.choice(np.arange(len(pdf)), p=pdf)
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        for t in range(len(observations) - 1, -1, -1):
            ...
            L_theta -= G * torch.log(self.policy.forward(torch.from_numpy(observations[t]).float())[actions[t]])
        L_theta = L_theta / len(observations)
        #perform a gradient descent step
        self.policy_optim.zero_grad()
        L_theta.backward()
        self.policy_optim.step()
        p_loss = L_theta.detach().numpy()
