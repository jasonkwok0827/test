        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        if random.random() < self.epsilon:
            ...
            for a in range(self.n_acts):
                if self.q_table[(obs, a)] > max_q:
                    max_q = self.q_table[(obs, a)]
                    max_a = a
            return max_a
        ###END###
...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        #Q-learning update
        gamma,alpha,q_table,n_acts = self.gamma, self.alpha,self.q_table,self.n_acts
        if done:
            q_table[(obs, action)] = q_table[(obs, action)] + alpha * (reward - q_table[(obs, action)])
        else:
            q_table[(obs, action)] = q_table[(obs, action)] + alpha * (reward + gamma * max([q_table[(n_obs, a)] for a in range(n_acts)]) - q_table[(obs, action)])
        return q_table[(obs, action)]
        ### END ###
...
class MonteCarloAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        gamma,q_table,sa_counts = self.gamma,self.q_table,self.sa_counts
        # list(zip(obses, actions))
        state_action_pair = [(obses[i], actions[i]) for i in range(len(obses))]
        G = 0
        for t in range(len(obses) - 1, -1, -1):
            G = gamma * G + rewards[t]
            pair = state_action_pair[t]
            if pair not in state_action_pair[:t]:
                sa_counts[pair] = sa_counts.get(pair,0) + 1
                q_table[pair] += (G-q_table[pair]) / sa_counts[pair]
                updated_values[pair] = q_table[pair]
