...
class DDPG(Agent):
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        if explore:
            #define the noise
            eta = self.noise.sample()
            #get the action
            action = self.actor_target.forward(torch.from_numpy(obs).float()) + eta
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        y = rewards + (1 - done) * self.gamma * self.critic_target(torch.cat((next_states, self.actor_target(next_states)), dim=1))
        #update critic network
        self.critic_optim.zero_grad()
        loss_func_critic = torch.nn.functional.mse_loss(y, self.critic(torch.cat((states, actions), dim=1)))
        loss_func_critic.backward()
        self.critic_optim.step()
        #update actor policy
        self.policy_optim.zero_grad()
        loss_func_actor = torch.mean(-self.critic(torch.cat((states, self.actor(states)), dim=1)))
        loss_func_actor.backward()
        self.policy_optim.step()
        #update target networks with soft updates
        self.critic_target.soft_update(self.critic, self.tau)
        self.actor_target.soft_update(self.actor, self.tau)

        q_loss = loss_func_critic.detach().numpy()
        p_loss = loss_func_actor.detach().numpy()
