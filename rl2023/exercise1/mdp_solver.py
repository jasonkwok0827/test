...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        #compile mdp
        self.mdp._compile()
        P, R, state_dim, action_dim, gamma = self.mdp.P, self.mdp.R, self.state_dim, self.action_dim, self.gamma
        while True:
            #initialize delta
            delta = 0
            for s in range(state_dim):
                v = V[s]
                #update V[s]
                V[s] = np.amax([np.sum([P[s,a,s1]*(R[s,a,s1]+gamma*V[s1]) for s1 in range(state_dim)]) for a in range(action_dim)])
                delta = np.amax([delta,abs(v-V[s])])
            if delta<theta:
                break
        return V
        ###END###
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for s in range(state_dim):
            policy[s, np.argmax([np.sum([P[s,a,s1]*(R[s,a,s1]+gamma*V[s1]) for s1 in range(state_dim)]) for a in range(action_dim)])] = 1
        return policy
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s in range(state_dim):
                ...
                #update V[s]
                V[s] = np.sum([P[s,a,s1]*(R[s,a,s1]+gamma*V[s1]) for s1 in range(state_dim)])
                delta = np.amax([delta,abs(v-V[s])])
            if delta<self.theta:
                break
        return np.array(V)
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            A_ = policy.copy()
            for s in range(state_dim):
                #set deterministic action = 1, others = 0
                ind = np.argmax([np.sum([P[s,a,s1]*(R[s,a,s1]+gamma*V[s1]) for s1 in range(state_dim)]) for a in range(action_dim)])
                policy[s,:] = 0
                policy[s,ind] = 1
            if np.array_equal(A_,policy):
                break
        return policy, V
